$(document).on('submit', '.archivarForm', function (event) {
    ...
    var form = $(this);
    var button = form.find('[type="submit"].active');
    ...
    // Mostrar una ventana de confirmación antes de continuar
    Swal.fire({
        title: 'Confirmación',
        ...
        if (result.isConfirmed) {
            // Si el usuario hace clic en "Sí, continuar", ejecuta la solicitud AJAX
            $.ajax({
                type: 'POST',
                url: '/archivar_publicacion',
                data: {
